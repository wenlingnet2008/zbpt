...
@section('content')
    ...
    <form method="post" action="{{ route('admin.livelists.store') }}" onsubmit="return Dcheck();" enctype="multipart/form-data">
        {{csrf_field()}}
        <table cellspacing="0" class="tb">
            <tr>
                <td class="tl"><span class="f_red">*</span> 直播名称</td>
                <td><input type="text" size="20" name="live[name]" id="name" value="@if(old('live.name')){{old('live.name')}}@endif" />&nbsp;<span
            ...
            <tr>
                <td>直播开始时间</td>
                <td><input type="text" size="20" name="live[start_time]" id="start_time" value="@if(old('live.start_time')){{old('live.start_time')}}@endif" />&nbsp;</td>
            </tr>

            <tr>
                <td>直播结束时间</td>
                <td><input type="text" size="20" name="live[end_time]" id="end_time" value="@if(old('live.end_time')){{old('live.end_time')}}@endif" />&nbsp;</td>
            </tr>
