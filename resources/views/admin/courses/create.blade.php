...
@section('content')
    ...
    <form method="post" action="{{ route('admin.courses.store') }}" onsubmit="return Dcheck();" enctype="multipart/form-data">
        {{csrf_field()}}
        <table cellspacing="0" class="tb">
            <tr>
                <td class="tl"><span class="f_red">*</span> 课程名称</td>
                <td><input type="text" size="20" name="course[name]" id="name" value="@if(old('course.name')){{old('course.name')}}@endif" />&nbsp;<span
            ...
            <tr>
                ...
                <td><textarea name="course[content]" id="content" class="dsn">@if(old('course.content')){{old('course.content')}}@endif</textarea>
