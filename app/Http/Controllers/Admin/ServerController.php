...
class ServerController extends Controller
{
    ...
    {
        if($service == 'chat'){
            exec("/usr/bin/env php ".base_path('gatewayworker/start.php')." restart -d >> /dev/null");
            return redirect()->route('admin.server')->with(['status'=>'重启聊天服务成功']);
        }elseif ($service == 'robot'){
            exec('/usr/bin/env php '.base_path('artisan').' workerman:client restart --daemonize >> /dev/null');
